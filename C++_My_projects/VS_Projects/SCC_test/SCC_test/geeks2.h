...
void Graph::findSCCs()
{
	...
	while (Stack->empty() == false) /// (2)
	{
		...
		if (visited[v] == false)
		{
			...
			if (counter > max_scc[1]) {
				max_scc[1] = counter;
				if (max_scc[1] > max_scc[2]) {
					swap(max_scc[1], max_scc[2]);
					if (max_scc[2] > max_scc[3]) {
						swap(max_scc[2], max_scc[3]);
						if (max_scc[3] > max_scc[4]) {
							swap(max_scc[3], max_scc[4]);
