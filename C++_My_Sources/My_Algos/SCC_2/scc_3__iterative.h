#ifndef SCC_3__ITERATIVE_H_INCLUDED
#define SCC_3__ITERATIVE_H_INCLUDED
...
/// 04.11.17 - (4) fillOrder function changed from recursive to iterative
...
class Graph
{
    ...
    int* checks;
    int* prevs; ///array of numbers of previous nodes
    //bool* checked_all;
...
Graph::Graph(int V)
{
    ...
    checks = new int[V+1];
    for (int i = 1; i <= V; ++i) {checks[i] = 0;}
    //checked_all = new bool[V+1];
    //for (int i = 1; i <= V; ++i) {checked_all[i] = 0;}
    prevs = new int[V+1];
    for (int i = 1; i <= V; ++i) {prevs[i] = 0;}
}
...
void Graph::fillOrder(int v, bool visited[], stack<int>* Stack)/// <<--- ERROR!!! ///(2) - was stack<int>& Stack
{
    ...
    visited[v] = true;
    ///stack<int> current_sequence;/// to help track previous vertices
    int current_vert = v;

    /// Iteration for all the vertices adjacent to this vertex
    while (checks[v] < degs[v])
    {
        /// First we check if we marked all of it's neighbors and if not -
        /// change current node/vert to unchecked neighbor, and step into the following iteration
        /// else - we step back (if all neighbors are already marked)
        if (checks[current_vert] == degs[current_vert]) {
            int previous = prevs[current_vert];
            Stack->push(current_vert);
            current_vert = previous;
            checks[current_vert]++;
        } else {
          int next_neighb = adj[current_vert][checks[current_vert]];
          if (visited[next_neighb]) {checks[current_vert]++;} else
          {
              if (!visited[next_neighb]) {
                  ///current_sequence.push(current_vert);
                  visited[next_neighb] = true;
                  prevs[next_neighb] = current_vert;
                  //checks[current_vert]++;
                  current_vert = next_neighb;
              }
          }
        }
    }
...
#endif // SCC_3__ITERATIVE_H_INCLUDED
